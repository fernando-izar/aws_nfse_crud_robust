...
export default function App() {
  const [apiUrl, setApiUrl] = useState<string>(
    () => localStorage.getItem(`${KEY}:apiUrl`) || ""
  );
  const [idToken, setIdToken] = useState<string>(
    () => localStorage.getItem(`${KEY}:idToken`) || ""
  );
  const [apiKey, setApiKey] = useState<string>(
    () => localStorage.getItem(`${KEY}:apiKey`) || ""
  );
  ...
  useEffect(() => {
    localStorage.setItem(`${KEY}:apiUrl`, apiUrl);
  }, [apiUrl]);
  useEffect(() => {
    localStorage.setItem(`${KEY}:idToken`, idToken);
  }, [idToken]);
  useEffect(() => {
    localStorage.setItem(`${KEY}:apiKey`, apiKey);
  }, [apiKey]);
  const api = useMemo(
    () => apiBuilder(apiUrl, idToken, apiKey),
    [apiUrl, idToken, apiKey]
  );
  const ping = async () => {
    try {
  ...
  return (
    ...
    >
      <h1>NFS-e Admin (Robusta r2)</h1>
      <p>Preencha API URL, API Key e id_token (Hosted UI).</p>
      ...
      >
        ...
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            const d = prompt(
              "Cognito domain (https://<dom>.auth.<reg>.amazoncognito.com)"
            );
            const c = prompt("ClientId");
            const cb = prompt("Callback (ex: http://localhost:5173/)");
            if (d && c && cb) {
              location.href = `${d}/login?client_id=${c}&response_type=token&scope=openid+email+profile&redirect_uri=${encodeURIComponent(
                cb
              )}`;
            }
          }}
